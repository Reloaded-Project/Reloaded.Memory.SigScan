...
namespace Reloaded.Memory.SigScan.Tests
{
    public class ScannerTests
    {
        ...
        {
            
        };

        private readonly int[] _data2ExpectedOffsetsForBasic = {0x13, 0x33, 0x53, 0x73, 0x93, 0xB3, 0xD3, 0xF3};
        private readonly int[] _data2ExpectedOffsetsForMasked = {0x00, 0x20, 0x40, 0x60, 0x80, 0xA0, 0xC0, 0xE0};
        ...
        {
            var scanner = new Scanner(_data1);
        ...
        [Fact]
        public void FindAllBasicPatterns()
        {
            var scanner = new Scanner(_data2);
            var resultCompiled = scanner.CompiledFindAllPatterns("A3 B4 C5 D6 E7 F8 09 1A 2B 3C 4D 5E 6F");
            var resultSimple = scanner.SimpleFindAllPatterns("A3 B4 C5 D6 E7 F8 09 1A 2B 3C 4D 5E 6F");

            Assert.Equal(resultCompiled, resultSimple);
            Assert.Equal(_data2ExpectedOffsetsForBasic, resultCompiled);
        }
        ...
        [Fact]
        public void FindAllPatternsWithMask()
        {
            var scanner = new Scanner(_data2);
            var resultCompiled = scanner.CompiledFindAllPatterns("48 8D 15 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E9 ?? ?? ?? ??");
            var resultSimple = scanner.SimpleFindAllPatterns("48 8D 15 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E9 ?? ?? ?? ??");

            Assert.Equal(resultCompiled, resultSimple);
            Assert.Equal(_data2ExpectedOffsetsForMasked, resultCompiled);
        }
